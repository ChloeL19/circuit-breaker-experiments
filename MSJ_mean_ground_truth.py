...
def convert_sample_to_mistral_prompts(
    ...
    # turns = sample.lstrip("\n").split("\n\n") # FIXME: there are some newlines that are not followed by Human: and I don't want to split on those newlines
    turns = re.split(r'\n\n(?=Human:|Assistant:)', sample.lstrip("\n"))  # Split only on newlines followed by "Human:" or "Assistant:"
...
for model_name in model_names_to_max_context_lengths_dict.keys():
    ...
    # i think somewhere below here I should get the ids of the response tokens
    # for every prompt, store in a list
